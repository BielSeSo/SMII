...
Mat imgOrg, imgUmb, imgHist, imgObj; 
int umbral_value = 0;
int umbral_bar = 3;
...
int procesarImg( Mat imgOrg, Mat imgUmb, int valor );
...
void Op_centro_de_gravedad(Mat* origen, Mat* destino);
...
String rute;
    ...
    {
        printf("El comando debe ser: ./bin/ejercico_9 <Ruta a la imagen>\n");
        return -1;
    }

    imgUmb.create(IMG_SIZE, IMG_SIZE, CV_8UC1);
    imgObj.create(IMG_SIZE, IMG_SIZE, CV_8UC1);
...
void umbralize_img(Mat* origen, Mat* destino)
{
    ...
    cvtColor(*origen, grays, COLOR_BGR2GRAY);
    threshold(grays, *destino, umbral_value, 255, THRESH_BINARY);
}
...
{ 
  ...
  switch( event ) {
    case EVENT_LBUTTONDOWN: 
      ...
      if (umbralize)
      {
        umbralize_img(&imgOrg, &imgUmb);
        imshow( fDST, imgUmb);

        Op_centro_de_gravedad(&imgUmb, &imgObj);
        imshow( fOBJ, imgObj);
...
void Op_centro_de_gravedad(Mat* origen, Mat* destino)
{
    ...
    *destino = *origen;
    ...
    cv::Moments m = cv::moments(*origen, true);
